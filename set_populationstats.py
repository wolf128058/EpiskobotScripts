...
try:
    populationpos = list_headlines.index(str.encode('Total Population'))
except:
    print('!-- Column for Total Population not found')
    exit()
...
for html_row in stat_html_rows:
    ...
    try:
        ...
        my_population = int(locale.atoi(td_in[populationpos].decode('utf-8')))
        if my_population == 0:
            continue
        a_yearstat_ch[str(my_year)] = my_population
    except:
        continue


try:
    known_stats = itemdetails['claims']['P1082']
except KeyError:
    known_stats = []
...
for known_stat in known_stats:
    ...
    my_population = trgt_number.amount
    a_yearstat_wd[str(my_year)] = int(my_population)
...
for key in a_todo_stat:
    claim_populationnumber = pywikibot.Claim(repo, 'P1082')
    amount_populationnumber = pywikibot.WbQuantity(a_todo_stat[key], site=wikidata_site)
    claim_populationnumber.setTarget(amount_populationnumber)
    quali_pit = create_claim_pit(int(key))
    claim_populationnumber.addQualifier(quali_pit)

    source_claim = pywikibot.Claim(repo, 'P1866')
    source_claim.setTarget(dioid)
    claim_populationnumber.addSources([source_claim])
    target_page.addClaim(claim_populationnumber, summary='added population amount')
