...
SELECT ?item ?itemLabel ?birthLabel ?cathiLabel WHERE {
 ...
 FILTER( ?position NOT IN (wd:Q1144278, wd:Q948657) )
...
for item in generator:
    ...
    for cathids in claim_list_cathid:
        ...
        if r.status_code != 200:
            ...
            continue
        diocesan_bishopship_tr = re.findall(r"\<tr\>\<td[^\>]+\>.*\<\/td\>\<td\>.*\<\/td\>\<td\>Bishop of (.*)\<\/td\>.*\<\/tr\>", r.content)
        titular_bishopship_tr = re.findall(r"\<tr\>\<td[^\>]+\>.*\<\/td\>\<td\>.*\<\/td\>\<td\>Titular Bishop of (.*)\<\/td\>.*\<\/tr\>", r.content)
        ...
        if len(diocesan_bishopship_tr) > 0:
            for x in range(0, len(diocesan_bishopship_tr)):
                dioceseid = re.findall(r'href="\/diocese\/d([a-z0-9]+).html"', diocesan_bishopship_tr[x])
                if(len(dioceseid) > 0):
                    l_dbishop.append(dioceseid[0])
        ...
        if len(titular_bishopship_tr) > 0:
            for y in range(0, len(titular_bishopship_tr)):
                dioceseid = re.findall(r'href="\/diocese\/d([a-z0-9]+).html"', titular_bishopship_tr[y])
                if(len(dioceseid) > 0):
                    l_tbishop.append(dioceseid[0])
