...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            print('set_dbishopstartdates.py -d <diocese> -p <position>')
            sys.exit()
...
list_events = ['Appointed', 'Selected', 'Succeeded', 'Confirmed']
list_pos = {}
...
if position == False and diocese != False:
    for single_pos in list_pos.keys():
        os.system('python3 set_dbishopstartdates.py -d ' + diocese + ' -p' + single_pos)
...
with progressbar.ProgressBar(max_value=len(generator), redirect_stdout=True) as bar:
    for index, item in enumerate(generator):
        ...
        if l_dbishopships_wostart:
            ...
            print('### No selected position w/o startdate!')
            continue
        ...
        for single_pos in l_dbishopships_wostart:
            ...
            if my_chdid:
                ...
                if bishop_tr and len(bishop_tr) == 1:
                    ...
                    print('-- No Pos-Start Data in table found. I skip.')
                    continue
