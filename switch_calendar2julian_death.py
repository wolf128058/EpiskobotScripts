...
SELECT ?item ?date WHERE {
  ?item (p:P570/psv:P570) ?datevalue.
...
with progressbar.ProgressBar(max_value=len(generator), redirect_stdout=True) as bar:
    for index, item in enumerate(generator):
        ...
        try:
            claim_list_died = itemdetails['claims']['P570']
        except:
            print('-- No Death-Date found. I skip that one')
            continue

        claim_deathdate = pywikibot.Claim(repo, 'P570')

        for single_date in claim_list_died:
