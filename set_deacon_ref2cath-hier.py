...
def main():
    ...
    parser.add_argument('-s', '--select', default='empty',
                        help=r'select entries with empty/wp/all references')
    ...
    with progressbar.ProgressBar(max_value=len(generator), redirect_stdout=True) as bar:
        ...
        for index, item in enumerate(generator):
            ...
            for job_claim in claim_list_jobs:
                ...
                if trgt.id == 'Q25393460':
                    job_claim_sources = job_claim.getSources()

                    if len(job_claim_sources) == 0:
                        r = requests_retry_session().get(chorgurl)
                        deacon_tr = re.findall(
                            b"<tr><td[^>]+>(.*)</td><td>.*</td><td>Ordained Deacon</td>.*</tr>", r.content)

                        if r.status_code == 200 and len(deacon_tr) > 0:
                            print(deacon_tr)
                            print('-- Status 200 received for: ' + chorgurl)
                            source_claim_statedin = pywikibot.Claim(
                                repo, 'P248')
                            source_claim_statedin.setTarget(
                                pywikibot.ItemPage(repo, 'Q3892772'))
                            source_claim_catid = pywikibot.Claim(repo, 'P1047')
                            source_claim_catid.setTarget(mycathid)
                            source_claim_retrieved = pywikibot.Claim(
                                repo, 'P813')
                            ...
                            job_claim.addSources([source_claim_statedin, source_claim_catid, source_claim_retrieved],
                                                 summary='add catholic-hierarchy as source for beeing catholic deacon')
                        ...
                        for job_claim_source in job_claim_sources:
                            count_src += 1
                            print('-- Found claim for beeing catholic deacon')
                            if 'P1047' in job_claim_source:
                                ...
                                if SELECT == 'all':
                                    try:
                                        ...
                                        job_claim.removeSources(
                                            [claimTarget[0]], summary='remove source (will be replaced immediately, if still valid)')
                                        cath_id_src = False
                                    except:
                                        print(
                                            '!! - Error on source-claim removal')
                            else:
                        ...
                        if not cath_id_src and (SELECT == 'all' or (SELECT == 'wp' and count_src == wp_found)):
                            ...
                            if r.status_code == 200 and len(deacon_tr) > 0:
                                ...
                                source_claim_statedin.setTarget(
                                    pywikibot.ItemPage(repo, 'Q3892772'))
                                source_claim_catid = pywikibot.Claim(
                                    repo, 'P1047')
                                source_claim_catid.setTarget(mycathid)
                                source_claim_retrieved = pywikibot.Claim(
                                    repo, 'P813')
