...
SELECT ?item WHERE {
  ...
  FILTER(NOT EXISTS { ?item wdt:P106 wd:Q25393460. })
}
"""

path4qs = 'data/log_quick_hiddendeacons.txt'
    ...
    for index, item in enumerate(generator):
        ...
        for cathids in claim_list_cathid:
            ...
            if r.status_code != 200:
                ...
                continue
            deaconship_tr = re.findall(b"<tr\><td[^>]+>.*</td><td>.*</td><td>Ordained Deacon</td>.*</tr>", r.content)
            ...
            for single_pos in claim_list_currentjobs:
                ...
                if trgt_pos.id == 'Q25393460':
                    l_known_deaconships.append(single_pos)

            if (len(l_known_deaconships) > 0):
                print('-- Found {} Job-Claims for Deaconships on WD'.format(len(l_known_deaconships)))

            if (len(deaconship_tr) > 0):
                print('-- Found {} Job-Claims for Deaconships on CH'.format(len(deaconship_tr)))

            if (len(deaconship_tr) > 0 and len(l_known_deaconships) == 0):
                item_properties += "\n" + mywd_id + "\tP106\tQ25393460"
